...
namespace DemacMedia\Bundle\CustomSalesBundle\Model\Action;
...
class RequestEntityCustom extends AbstractAction
{
    /** @var array */
    protected $options;
    ...
    protected function getEntityByConditions($context)
    {
        ...
        foreach ($where as $field => $value) {
            ...
            if ($field == "e.createdAt" || $field == "e.updatedAt"){
                $date24HoursAgo = (new \DateTime())->modify('-24 hours');
                $date24hoursAgoString = sprintf("'%s'", $date24HoursAgo->format('Y-m-d H:i:s'));
                $queryBuilder->andWhere($queryBuilder->expr()->gte($field, $date24hoursAgoString));
            } else {
